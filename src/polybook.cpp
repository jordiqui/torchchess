...
void PolyBook::init(const OptionsMap& options) {
    ...
    polybook[2].init(options["Book3 File"]);
    polybook[3].init(options["Book4 File"]);
    polybook[4].init(options["Book5 File"]);
    polybook[5].init(options["Book6 File"]);
}
...
void PolyBook::init(const std::string& bookfile) {
    ...
    std::FILE* fpt = std::fopen(bookfile.c_str(), "rb");
    if (fpt == nullptr)
    {
    ...
    errno = 0;
    if (std::fseek(fpt, 0L, SEEK_END) != 0)
    {
        sync_cout << "info string Could not seek to end of " << bookfile << ": "
                  << std::strerror(errno) << sync_endl;
        std::fclose(fpt);
        return;
    }

    long tell = std::ftell(fpt);
    if (tell < 0)
    {
        sync_cout << "info string Could not determine size of " << bookfile << ": "
                  << std::strerror(errno) << sync_endl;
        std::fclose(fpt);
        return;
    }

    std::size_t filesize = static_cast<std::size_t>(tell);

    if (std::fseek(fpt, 0L, SEEK_SET) != 0)
    {
        sync_cout << "info string Could not rewind " << bookfile << ": "
                  << std::strerror(errno) << sync_endl;
        std::fclose(fpt);
        return;
    }
    ...
    if (filesize < sizeof(PolyHash) || keycount == 0 || filesize % sizeof(PolyHash) != 0)
    {
        if (polyhash)
        {
            std::free(polyhash);
    ...
    errno                   = 0;
    std::size_t readSize    = std::fread(polyhash, 1, filesize, fpt);
    std::fclose(fpt);
    ...
    if (readSize != filesize)
    {
        ...
        sync_cout << "info string Could not read " << bookfile << ": "
                  << std::strerror(errno) << sync_endl;
        return;
    }
